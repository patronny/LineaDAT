...
Глубокий разбор `wBTCStrategy` на Ethereum mainnet. Это **прямой прототип LINEASTR** — мы форкаем v3-сурсы с MIT-атрибуцией, переcalibrated под Linea. Все факты ниже подтверждены RPC-вызовами через `https://eth.drpc.org` и сырыми receipt'ами / call-trace'ами в [`research/raw-rpc-data/`](../research/raw-rpc-data/) (2026-05-01).
...
**Для LINEASTR этот блок переименовываем** в LINEASTR-burn, с edge-case: пока `collection == LINEASTR_ADDRESS` — `lineastrBurnAmount` redirected в `feeAddress` ⇒ эффективно **80/20** на самом $LINEASTR. Для будущих strategies — нормальный 80/10/10 split (см. [`50-lineastr-spec.md`](50-lineastr-spec.md)).
...
В LINEASTR заменим параметр `_punkStrategy` на `_lineastrAddress` — но смысл тот же (адрес токена, чьи проценты идут в buy-and-burn).
