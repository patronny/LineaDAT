...
## 8. На что смотрим как на референс для LINEASTR

✅ **Storage layout** — копируем 1:1 (это интерфейс с RPC и indexer'ами, любые отклонения сломают frontend).
✅ **`buyIncrement = 0.1 ETH/блок`** — TokenWorks нашли это эмпирически, v2 уже использует. Подтверждение что 0.1 ETH/блок — норма для mainnet (12с/блок). Для Linea (3с/блок) пересчитываем: см. [`docs/50-lineastr-spec.md`](50-lineastr-spec.md).
...
✅ **`bagSize`** — у REKTSTR это 42 069 000 000 REKT (мем-число). Это даёт нам подтверждение, что bagSize **может быть произвольным круглым** числом — не привязано к % от supply underlying. Для LINEASTR выбрали **150 000 LINEA** как «удобное круглое».

## 9. Что забираем как "v2 был баги-первенец, v3 уже почистили"

REKTSTR — **первый ERC-20 strategy** TokenWorks. На нём могли быть обкатаны фиксы для v3. Конкретные публично известные баги REKTSTR — не зафиксированы в открытых каналах (см. [`30-tokenworks-incidents.md`](30-tokenworks-incidents.md)), но факт что v3 деплоился через **месяц** после v2 говорит, что было что-то правлено. **LINEASTR форкаем именно v3** — берём пост-фикс версию.
