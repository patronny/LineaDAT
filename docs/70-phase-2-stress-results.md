...
| Final totalSupply (LINEASTR) | 1 000 000 000 (без изменений — processTokenTwap stub) |
...
- ❌ `processTokenTwap` execution — наш `MockUniversalRouter` не возвращает LINEASTR при swap, поэтому `_buyAndBurnTokens` корректно не отработает. Это тестируется отдельно в `Sandwich.t.sol` с controlled mock router.
...
1. Deploy полной LINEASTR-инфраструктуры на Base Sepolia
