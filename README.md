...
10% trade-fee на каждом swap → **80% treasury** (накопление ETH под выкуп $LINEA через P2P-оффер) + **20% создателю** (через redirect 10% LINEASTR-burn в feeAddress пока collection == сам $LINEASTR) → автоматический yoyo-цикл buy/relist 1.2× → buy-and-burn LINEASTR через `processTokenTwap`.

## Чем отличается от прототипа wBTCStrategy

| Аспект | wBTCStrategy (прототип v3) | LINEASTR |
|---|---|---|
...
| Initial pool | single-sided 0 ETH + 1B WBTCSTR | **single-sided 0 ETH + 1B LINEASTR** |
...
| Effective fee split | 90% treasury / 10% PNKSTR-burn / 0% feeAddress (т.к. `feeAddressClaimedByOwner=0`) | **80% treasury / 10% LINEASTR-burn-redirected-to-creator / 10% creator** = **80/20 эффективно** |
| LINEASTR-burn block | PNKSTR-burn (hard-coded) | **LINEASTR-burn (если collection ≠ LINEASTR_ADDRESS), else в feeAddress** |
...
🟢 **Этап 1 — спецификация согласована.** Все параметры залочены (см. [`docs/50-lineastr-spec.md`](docs/50-lineastr-spec.md)). Контракты пока не написаны — это следующий этап.
...
- [`docs/50-lineastr-spec.md`](docs/50-lineastr-spec.md) — **финальная спека LINEASTR** (контракты, параметры, fee, бот, UI)
