...
Сырые данные с Ethereum / Linea RPC, собранные при research-проходе 2026-05-01. Используются как источник истины для анатомий REKTSTR / WBTCSTR и для аналитики $LINEA на Linea.
...
- **WBTCSTR LP-NFT (PositionManager):** tokenId 132829 → minted to `0x0...dead` ⇒ ликвидность залочена навсегда
- **fee split в WBTCSTR (фактический):** 90% treasury / 10% PNKSTR-burn / 0% feeAddress (потому что `feeAddressClaimedByOwner[WBTCSTR]=0` ⇒ ownerAmount сливается в treasury)

### Проверочные команды (можно повторить)
