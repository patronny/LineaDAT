...
import {LineastrSeeder} from "../src/LineastrSeeder.sol";
...
///
/// SEQUENCE:
///   1. Deploy new LINEASTRStrategy implementation (with factoryEscape function)
///   2. UUPS upgrade the existing proxy to the new impl
///   3. Deploy LineastrSeeder
...
contract SeedLiquidityBaseSepolia is Script {
    ...
    int24 constant TICK_UPPER = 180000;  // approx 1 ETH ≈ 6.6e7 LINEASTR (FDV ~$66M with 1B supply)
    ...
    // For single-sided LP at currentTick == tickUpper, all currency1 (LINEASTR), no currency0 (ETH).
    ...
    function run() external {
        ...
        // === Step 1: Deploy new LINEASTRStrategy impl ===
        vm.startBroadcast();

        LINEASTRStrategy newImpl = new LINEASTRStrategy();
        console.log("[1] New LINEASTRStrategy impl:", address(newImpl));
        ...
        LineastrSeeder seeder = new LineastrSeeder(IPoolManager(POOL_MANAGER));
        console.log("[3] LineastrSeeder:", address(seeder));

        // === Step 4: Drain factory's tokens to seeder ===
        proxy.factoryEscape(address(seeder), TOTAL_SUPPLY);
        console.log("[4] factoryEscape -> seeder. seeder LINEASTR balance:", proxy.balanceOf(address(seeder)));

        // === Step 5: Set loadingLiquidity flag on factory ===
        LINEASTRFactory factory = LINEASTRFactory(payable(FACTORY));
        ...
        console.log("Pool:    ETH/LINEASTR, fee=DYNAMIC, hook=", HOOK);
