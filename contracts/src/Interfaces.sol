...
// Trimmed for LINEASTR: removed NFT/ERC-721/ERC-1155/PunkStrategy interfaces (not used).
...
/// @title LINEASTR core interfaces
/// @notice Interfaces shared between LINEASTRStrategy / BaseStrategy / LINEASTRHook / LINEASTRFactory.
...
/// @notice Common strategy interface — shared between LINEASTR and any future LINEASTR-family strategies on Linea.
...
/// @notice Specialized interface for ERC-20 backed strategies (the only kind LINEASTR supports).
interface ILineastrStrategy is IStrategy {
    function initialize(
        address _token,
        uint256 _bagSize,
        address _hook,
...
/// @notice LINEASTR factory interface — extends `IBaseStrategyFactory` with strategy-token mapping.
interface ILineastrFactory is IBaseStrategyFactory {
